...
export const metadata = {
  title: 'Penyesuaian Stok - CatStock',
  description: 'Sesuaikan tingkat inventori berdasarkan perhitungan fisik'
}
...
export default async function StockAdjustmentRoute() {
  ...
  try {
    ...
    console.log('[Stock Adjustment] Quick auth result:', !!session?.isAuthenticated)
  } catch (error) {
    console.log('[Stock Adjustment] Quick auth failed, trying regular auth')
  }
  ...
  if (!session?.isAuthenticated) {
    try {
      ...
      console.log('[Stock Adjustment] Regular auth result:', !!session?.isAuthenticated)
    } catch (error) {
      console.log('[Stock Adjustment] Regular auth also failed')
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">Silakan masuk untuk mengakses penyesuaian stok.</p>
