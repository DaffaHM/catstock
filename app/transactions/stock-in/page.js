...
export const metadata = {
  title: 'Stok Masuk - CatStock',
  description: 'Terima inventori dari pemasok'
}

export default async function StockInTransactionPage() {
  let session = null
  
  // Try quick authentication first (for demo mode)
  try {
    session = await getQuickSession()
    console.log('[Stock In] Quick auth result:', !!session?.isAuthenticated)
  } catch (error) {
    console.log('[Stock In] Quick auth failed, trying regular auth')
  }
  ...
  if (!session?.isAuthenticated) {
    try {
      ...
      console.log('[Stock In] Regular auth result:', !!session?.isAuthenticated)
    } catch (error) {
      console.log('[Stock In] Regular auth also failed')
    }
  }
  ...
  return <StockInPage isDemoMode={isDemoMode} />
}