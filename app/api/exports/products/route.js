...
import { getProductsForExport } from '@/lib/queries/csv-exports'
import { 
  handleCsvExport, 
...
export async function GET(request) {
  try {
    ...
    const filters = {
      ...
      includeStockLevels: searchParams.get('includeStockLevels') !== 'false'
    }
    ...
    const result = await handleCsvExport(
      ...
      {
        filename: createExportFilename('products', filters)
      }
    )
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      ...
      message: `Successfully exported ${result.rowCount} products`
    })
  } catch (error) {
    console.error('Products export POST API error:', error)
    return NextResponse.json(
      { error: 'Export failed due to server error' },
      { status: 500 }
    )
