...
import { getStockMovementsForExport } from '@/lib/queries/csv-exports'
import { 
  handleCsvExport, 
...
export async function GET(request) {
  try {
    ...
    const result = await handleCsvExport(
      CSV_EXPORT_CONFIG.EXPORT_TYPES.STOCK_MOVEMENTS,
      filters,
      getStockMovementsForExport,
      {
        filename: createExportFilename('stock-movements', filters)
      }
    )
    ...
    console.error('Stock movements export API error:', error)
    return NextResponse.json(
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      ...
      message: `Successfully exported ${result.rowCount} stock movements`
    })
  } catch (error) {
    console.error('Stock movements export POST API error:', error)
    return NextResponse.json(
      { error: 'Export failed due to server error' },
      { status: 500 }
    )
