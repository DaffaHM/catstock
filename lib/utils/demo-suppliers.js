...
export function getDemoSuppliers() {
  ...
  try {
    ...
    // Check if we have any stored suppliers
    let allSuppliers = []
    ...
    if (stored) {
      ...
      if (Array.isArray(parsedSuppliers) && parsedSuppliers.length === 0) {
        console.log('[getDemoSuppliers] Reset state detected - returning empty array')
        return []
      }
      
      // Only add stored suppliers that are not deleted
      parsedSuppliers.forEach(supplier => {
        if (!deletedIds.includes(supplier.id)) {
          allSuppliers.push(supplier)
        }
      })
    } else {
      // No stored data exists - first time user, show base suppliers unless deleted
      BASE_DEMO_SUPPLIERS.forEach(supplier => {
        if (!deletedIds.includes(supplier.id)) {
          allSuppliers.push(supplier)
        }
      })
...
export function resetDemoSuppliers() {
  ...
  try {
    ...
    localStorage.setItem(DEMO_SUPPLIERS_KEY, JSON.stringify([]))
    localStorage.removeItem(DELETED_SUPPLIERS_KEY)
    console.log('Reset demo suppliers data - set to empty state')
    return true
  } catch (error) {
