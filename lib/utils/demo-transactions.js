...
export function createDemoTransaction(transactionData) {
  ...
  try {
    ...
    const transaction = {
      ...
      transactionDate: transactionData.transactionDate.toISOString(), // Ensure consistent date format
    ...
    if (stored) {
      ...
      if (Array.isArray(parsedData)) {
        existingTransactions = parsedData
      } else if (parsedData && Array.isArray(parsedData.transactions)) {
        existingTransactions = parsedData.transactions
      }
    }
...
export function resetDemoTransactions() {
  ...
  try {
    // Set empty array to indicate reset state (consistent with products and suppliers)
    localStorage.setItem(DEMO_TRANSACTIONS_KEY, JSON.stringify([]))
    console.log('Reset demo transactions data - set to empty state')
    return true
  } catch (error) {
