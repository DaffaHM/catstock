...
const BASE_DEMO_PRODUCTS = [
  ...
  },
  {
    ...
    size: '1',
    unit: 'Liter',
    paintColor: null, // No color for thinner
    purchasePrice: 25000,
    sellingPrice: 35000,
    minimumStock: 20,
    currentStock: 25,
  ...
  },
  {
    ...
    size: '2',
    unit: 'Inch',
    paintColor: null, // No color for brush
    purchasePrice: 15000,
    sellingPrice: 25000,
    minimumStock: 30,
    currentStock: 45,
...
export function getDemoProducts() {
  ...
  try {
    ...
    // Check if we have any stored products
    let allProducts = []
    ...
    if (stored) {
      ...
      // Only add stored products that are not deleted
      ...
      // No stored data exists - first time user, show base products unless deleted
      BASE_DEMO_PRODUCTS.forEach(product => {
        if (!deletedIds.includes(product.id)) {
          allProducts.push(product)
        }
      })
...
// Reset all demo data (clear localStorage and set empty state)
export function resetDemoProducts() {
  ...
  try {
    // Set empty array to indicate reset state
    localStorage.setItem(DEMO_PRODUCTS_KEY, JSON.stringify([]))
    localStorage.removeItem(DELETED_PRODUCTS_KEY)
    console.log('Reset demo products data - set to empty state')
    return true
  } catch (error) {
